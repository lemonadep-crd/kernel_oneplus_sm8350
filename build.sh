...
while true; do
	...
	echo -e "${BLUE}1.👉 Sakura_OP9${NC}"
	echo -e "${BLUE}2.👉 Custom git clone command${NC}"

	# Set timeout for user input (5 seconds)
	read -t 5 -rp "Enter your choice (1 or 2): " choice

	# If no input is provided within 5 seconds, default to action 1 (Sakura)
	[ -z "$choice" ] && echo -e "${YELLOW}No input detected. Automatically selecting Sakura.${NC}" && choice=1
